...
通过 Chukonu remote MCP 获取网页、学术和专利证据。本 skill 不调用本地二进制，也不需要 API key 或静态 token。只使用公开的 `search` 与 `research` 两个业务工具；陈述校验和 PDF 全文读取是 `research` 的内部阶段，不要调用 `verify_claims`、`get_pdf_text` 或内部上游服务，也不要直连 9001、9101 等内部端口。
