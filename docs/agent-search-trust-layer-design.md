...
> 状态：Evidence 标注与陈述校验内核已实现；`/search + /research` 目标用户接口待实现
>
...
3. 现有 `/verify`、`verify_claims` 和 `get_pdf_text` 不再作为目标公开能力；其逻辑分别下沉为 `/research` 的“陈述校验”和“全文深读”内部阶段。迁移期保留兼容入口。
...
- REST：`POST /search`、`POST /verify`、`GET /academic/pdf/text/{work_id}`；
- MCP：`search`、`verify_claims`、`get_pdf_text`；
- `/verify` 只评估调用方传入的 evidence，不主动补搜；
- PDF 续读返回裸正文 DTO，尚未形成可直接进入校验流程的标准 EvidenceUnit。

这些能力作为 `/research` 的内部构件继续保留，但不再定义目标用户心智。
...
因此，当前实现可以交付发现型 evidence 和保守的单次陈述评估，但在 `/research` 完成原文深读、可信 evidence 引用、主动反证和覆盖停止之前，不应把结果标为“完整验证”或“全面研究”。
...
|----------|----------------------|--------------|
| `/verify` / `verify_claims` | claim decomposition、entailment、consistency、source policy | 兼容期保留，最终不作为主入口 |
| `get_pdf_text` / REST PDF route | deep reading、content continuation、locator assembly | 兼容期保留，最终不作为主入口 |
...
参数解析顺序固定为 `profile defaults → depth budget preset → explicit objective/scope → policy registry → explicit budget cap → tenant/safety hard limits`。响应中的 `resolved` 是唯一实际执行口径，至少包含 normalized objective、resolved scope、seed 纳入/排除统计及原因、profile、depth、policy、budget、privacy 和 `adjustments[]`；被服务端收紧或拒绝的参数必须显式返回，不能静默改写。
...
它回答“这批搜索结果是否可作为进一步研究的种子”，不回答任何具体事实是否成立。顶层 `status=complete/partial` 只描述执行是否完整；成功执行但零结果仍是 `complete`，由 `retrieval_assessment.status=empty` 表达结果为空。当前按 evidence 数量推导 `answerable/high` 的逻辑属于兼容字段，目标响应中弃用。
...
`research_seed.search_id` 对调用者是不透明研究种子；它不能被用来枚举其他租户数据，必须绑定租户、权限、snapshot hash 和 TTL。只有 SeedStore 降级时 `research_seed` 才为 `null`。
...
REST 创建请求必须携带 `Idempotency-Key`，成功响应同时设置 `Location: /research/{id}` 与 `Retry-After`。MCP `operation=start` 使用必填的 `idempotency_key` 字段。服务端按 `tenant + key + canonical request hash` 去重：同 key、等价请求返回同一 task；同 key、不同请求返回 `409 IDEMPOTENCY_KEY_REUSED`，防止超时重试创建两份昂贵任务。

Seed 在创建时校验并复制到 ResearchTask：不存在或无权访问统一返回 `404 SEARCH_SEED_NOT_FOUND`，已过期返回 `410 SEARCH_SEED_EXPIRED`。任务一旦创建成功，不受后续 seed TTL 到期影响；来自 `partial` 执行或 `retrieval_assessment.status=empty` 的 seed 也允许启动，规划器负责扩展并在 dossier 披露初始缺口。
...
`standard/full` 响应中的每个 finding 引用都必须能在 `evidence_index` 解引用到 quote、locator、provenance、许可和版本。artifact 只用于导出和审计，不是核验 finding 的必要依赖；它与 task 使用相同租户、权限和 privacy policy，签名地址不得成为跨租户旁路。
...
MCP transport error 只表示工具调用本身未送达；已接受任务的业务 failure 必须通过 TaskEnvelope 返回。MCP 与 REST 使用相同 error code、租户身份、retention 和 artifact 权限。
...
主要映射：schema/字段错误 `400`，资源不存在或无权访问 `404`，幂等冲突/非法状态转移 `409`，已过期 seed/task `410`，语义或 policy/privacy 不可满足 `422`，配额/并发限制 `429`，所有计划来源不可用或基础设施不可用 `503`。外租户访问 ID 始终返回 `404`，不泄露资源是否存在。`restricted + external_processing=false` 若没有合规执行路径，在创建阶段返回 `422 PRIVACY_POLICY_UNSATISFIABLE`，不得静默外发或降低 policy。

Canonical 合同通过 `API-Version: 2026-07-17` 选择，并对未知字段严格拒绝。迁移期未携带该版本的旧客户端继续进入 legacy schema；旧 PDF/trust 参数属于 legacy 的已知 deprecated 字段，不与 strict V1 的“未知字段拒绝”矛盾。MCP 通过工具 schema 版本发布同一边界。

目标迁移规则：

1. `/search` 先增加 `research_seed/retrieval_assessment/retrieval_boundary`，保留旧字段一个兼容周期；
2. 新增 `/research` 任务资源和 MCP `research`；
3. `/verify` 与 `verify_claims` 改为调用 ResearchService 的单轮 `verification` 内部阶段，响应加 `Deprecation` 与迁移说明；
4. REST PDF route 与 `get_pdf_text` 改为调用 ResearchService 的 `deep_reading` 适配器，只为旧客户端保留；
5. Canonical `/search` 不再文档化 PDF/trust 高级参数，旧参数在兼容期继续解析并返回 deprecated warning；
6. 观测迁移完成后再设置 `Sunset`，不得在同一版本静默改变旧 `top_k` 或 response shape 的语义。
...
src/application/
  ...
  ports/
    search_seed_store.py    # /search 快速写入、/research 按 tenant 读取不可变 seed
...
| 文件 | 设计改动 |
|------|----------|
| `src/interfaces/schemas.py` | 新增 strict `SearchRequestV1`、`ResearchRequest` 和 task 操作 schema；禁用未知字段 |
| `src/interfaces/responses.py` | 新增 `SearchSeed`、`RetrievalAssessment`、`ResearchTaskResponse`、`ResearchDossier` |
...
| `src/engine.py` | 增加 `start_research/get_research` 门面；`verify/get_pdf_text` 仅兼容 |
| `src/api.py` | 新增 `/research` 资源；`/search` 使用 V1 canonical contract；旧路由返回弃用信息 |
| `src/mcp_server.py` | 稳定工具收敛为 `search/research`；旧工具进入兼容期 |
...
SearchSeedStore 是 `/search` 的轻量基础依赖，与研究 worker/模型隔离；ResearchStore 至少保存 task state、已复制的固定 seed、版本化 evidence set、执行轮次、实际查询/过滤/来源快照、policy/parser/model version 和 artifact 引用。MVP 可以先使用单机持久化队列与数据库；生产多实例必须使用共享持久化和具备租户隔离的 worker，不得依赖进程内内存保证研究任务可恢复。
...
- 至少一个检索分支完成时返回 `200`，并以顶层 `status=complete/partial` 区分完整执行和部分来源/过滤失败；成功执行但无结果使用 `complete + retrieval_assessment.status=empty`。
...
- `depth` 是普通调用者的成本预设；显式 budget 只能收紧服务端上限，不能绕过租户配额、许可、来源策略或人工复核策略。响应返回 `resolved.budget` 与实际 usage。
...
- `search_id/research_id/artifact` 全部绑定 tenant、principal 和权限；日志与 artifact 遵循 privacy mode，restricted 原文不得进入未授权外部模型或共享缓存。
- 小模型负责陈述拆解、字段抽取和候选定位；大模型只处理复杂蕴含、冲突分析与综合，按复杂度路由并记录实际模型版本。
- 每个租户限制并发研究数与排队时长；预计无法在任务 TTL/预算内启动时应在创建阶段明确拒绝，而不是无限期保持 `queued`。
...
9. 跨租户读取 seed/task、重复 Idempotency-Key、worker 重启、阶段重试与取消竞争。
...
- `POST /research` 具备幂等、租户隔离、持久化状态机、重启恢复、预算上限和可取消性；已接受任务不会因进程重启静默丢失。
...
- 当前 `/verify`、`verify_claims`、PDF route 和 `get_pdf_text` 视为迁移期能力与研究内部构件，不再继续扩展其公开契约。
- 本阶段能力仍受摘要、provider extract、初步 locator 和无主动补搜等限制，不能宣称已完成深度研究闭环。

### Phase 1：接口收敛与任务骨架

- 发布 strict `search.v1`：保留单轮轻量检索，生成不可变 `search_id`，返回 `retrieval_assessment` 与真实 filters/failures。
- 实现 SearchSeedStore、ResearchStore、持久化队列、ResearchTask 状态机、幂等创建、查询、反馈、取消、租户隔离和预算核算。
- 发布完整 `/research` 资源族与 MCP `research`；MVP 先编排现有 verify/PDF 能力，也必须输出 dossier、coverage gap、执行边界与停止原因。
- REST/MCP 同时发布弃用声明和迁移指南；旧入口不再增加新功能。
...
- 在旧接口使用率、错误率和迁移覆盖达到发布标准后再设置 Sunset；归档旧 schema 文档并保留兼容遥测。

## 14. 待确认事项

以下决策不改变两个接口的能力边界，但需要在 Phase 1 开发前固化：

| 决策 | 建议默认值 | 影响 |
|------|------------|------|
| SearchSeed 存储与 TTL | 共享持久化、默认 24 小时；TTL 写入响应 | 决定跨进程复现、存储成本和过期重搜体验 |
| Task/Artifact 保留期 | task 元数据长于原文 artifact；租户可配置删除策略 | 兼顾审计、许可、隐私和成本 |
