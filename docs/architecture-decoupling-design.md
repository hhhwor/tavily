...
> 状态：结构性解耦基线与 `/search + /research` 接口收敛已完成；当前进入研究深读增强、工程化与运行治理阶段
>
> 当前代码基线：2026-07-20，search.v1 / research.v1 实现工作树
>
...
2. `SearchEngine` 是 search/research 的薄门面；轻量发现位于 `DiscoveryService/SearchService`，异步研究位于 `ResearchService`。
...
6. REST 与 MCP 统一生成 `SearchCommand/ResearchCommand`，公开能力只保留 search 和 research。
7. SearchSeed 与 ResearchTask 使用 SQLite/WAL 持久化；任务固定复制 seed，并由独立 research executor 执行和恢复。
8. F-01 至 F-10、search.v1/research.v1 契约与研究生命周期由 147 个测试保护。

因此后续策略不再是“大规模重写”，而是继续按可独立验证、可独立提交的增量推进。现阶段最大风险已经从模块耦合转移到全文 locator 完整性、研究覆盖校准、工程可复现性和运行时治理。
...
| `src/mcp_server.py` | `search/research` tool schema、Command 映射与线程卸载 | 与 REST 共用应用命令和响应语义 |
| `src/engine.py` | search 与 ResearchTask 生命周期薄门面、资源关闭协调 | 不执行搜索/研究业务编排 |
| `src/application/` | Discovery、轻量 Search、Research 状态机、召回、排序、Evidence、Trust 与 Verify 内部阶段 | 公开用例的唯一编排层 |
| `src/application/ports/` | Retrieval、SearchSeedStore、ResearchStore、Cache、PDF、Entailment、Clock/Deadline 等边界 | 应用层外部依赖接口 |
| `src/domain/` | 不可变阶段文档、search.v1、research.v1、Evidence/Trust/Failure 模型和安全错误 | 不依赖 Web 框架、环境或外部 SDK |
...
| `src/infrastructure/` | SQLite stores、Cache、Clock、查询改写、PDF、HTTP 错误映射 | 外部运行机制实现 |
| `src/interfaces/` | strict REST/MCP 输入 schema 与响应别名 | 传输层不拥有业务状态机 |
| `src/trust/` | Provenance/Locator/Quality 标注、Claim 分解、规则与模型 Entailment | `/research` 内部可信阶段 |
| `src/models.py` | 领域模型 convenience exports | 生产模块按所有权直接导入 |
| `src/cache.py` | 旧 Cache 导入路径 re-export | 兼容门面 |
| `eval/` | IR、E2E、Agent 与 MCP 评测 | 请求和响应读取已迁移至 search.v1；研究评测继续扩充 |

### 2.2 当前依赖与装配方向

```mermaid
flowchart LR
    REST[FastAPI: search / research] --> CMD[SearchCommand / ResearchCommand]
    MCP[MCP: search / research] --> CMD
    CMD --> FACADE[SearchEngine]
    FACADE --> SEARCH[SearchService]
    FACADE --> RESEARCH[ResearchService]

    SEARCH --> DISCOVERY[DiscoveryService]
    DISCOVERY --> PLAN[QueryPlanner]
    DISCOVERY --> RECALL[RecallCoordinator]
    DISCOVERY --> RANK[RankingService]
    SEARCH --> EVIDENCE[EvidenceAssembler]
    SEARCH --> TRUST[TrustAnnotator]
    SEARCH --> ANSWER[AnswerabilityPolicy]
    SEARCH --> SEED[SearchSeedStore]

    RESEARCH --> DISCOVERY
    RESEARCH --> PDF[PdfTextGateway]
    RESEARCH --> VERIFY[VerifyService]
    RESEARCH --> TASKS[ResearchStore]
    RESEARCH --> WORKER[Dedicated Executor]
...
- REST/MCP 使用同一 strict schema 映射 `SearchCommand/ResearchCommand`，不保留公开 legacy 参数分支。
- Application 直接返回位于 `domain/search_api.py` 与 `domain/research.py` 的稳定合同，interfaces 只负责校验与序列化。

### 2.3 搜索主链路

1. REST `SearchRequest` 或 MCP tool 参数映射为冻结的 `SearchCommand(query/limit/source_types/filters)`。
2. `SearchEngine.execute` 只把 Command 委托给 `SearchService.execute`。
3. `DiscoveryService` 解析服务端固定 Ranking Profile，并协调 QueryPlanner、Recall 与 Ranking。
4. `QueryPlanner` 调用纯规则 L0 完成规范化、时效和领域识别；调用方不能覆盖模型或底层融合参数。
...
8. `/search` 不调用 PDF；`EvidenceAssembler` 构造 discovery evidence，`TrustAnnotator` 固定补充 Provenance、Locator 与 Quality。
9. `AnswerabilityPolicy` 被投影为 `RetrievalAssessment`；SearchService 记录逐类型过滤执行与真实 RetrievalBoundary。
10. SearchSeedStore 原子保存 evidence + boundary 快照并签发高熵 `search_id`；REST/MCP 直接序列化 `search.v1`。

单个搜索 `Deadline` 从 DiscoveryService 传播到 Recall 与 Ranking。Research 使用独立总 deadline，并把它传入后续 Discovery 与 PDF 深读；已经进入阻塞 HTTP 的任务仍依赖 adapter timeout 收敛。

### 2.4 Research 主链路

1. `POST /research` 按 `search_id` 读取 seed，解析 objective/scope/policy/budget，并将固定 seed 复制进 ResearchStore。
2. 独立 worker 按 scope 排除不合格 seed，执行多轮 Discovery、PDF 深读、identity 去重和主动反证查询。
3. `VerifyService` 委托 `ClaimVerifier` 分解原子陈述并匹配候选 Evidence。
4. `EntailmentClassifier` 批量判断 supports、contradicts、mentions、unclear 或 irrelevant；模型失败时回退保守规则并产生结构化 failure。
5. ResearchService 汇总 finding、EvidenceFunnel、CoverageMatrix、分维度 Assessment 和 Stop，原子提交新的 evidence/task revision。
...
### R-01 内部 legacy ranking 与 convenience exports 尚未清理（优先级：中）

公开 REST/MCP legacy 合同已经删除；仓库内部仍保留：

- `src/models.py` convenience exports
- `src/pipeline/rerank.py`
- `src/cache.py`
- `src/l0.py` 中查询改写兼容函数
- `ranking/legacy.py` 中 Fusion/Threshold/build_reranker

E2E/Agent/Tool Eval 已使用 search.v1 字段，但部分 Runner 仍从 `src.models` convenience path 导入；`eval/run_eval.py` 仍使用旧 rerank 路径。这些是仓库内部清理事项，不构成保留旧用户契约的理由。
...
当前 147 个测试能够保护排序规则、search/research 契约和任务生命周期，但还没有固定 corpus 的 Recall/NDCG、Evidence 字段和路由 golden baseline。领域权重、Provider 映射或 Evidence 截断调整仍可能在测试全绿时造成质量退化。
...
- [x] REST/MCP search.v1/research.v1 共享命令与契约。
- [x] shared model kernel 拆分和兼容导出。
- [x] 147 个测试通过。

### Phase B：配置与内部 legacy 收敛（下一阶段）

- [ ] 审计全部 Settings 字段，删除或接入无效开关。
- [x] E2E/Agent/Tool Eval 请求和响应迁移到 search.v1。
- [ ] 将 Eval 的 `src.models` convenience import 改为模型所有权路径。
- [ ] 删除 legacy Fusion/Threshold 构建路径前完成评测迁移。
- [x] 公共合同归入 `domain/search_api.py` 与 `domain/research.py`。

退出条件：生产与评测不再依赖内部 legacy ranking；所有公开配置均被消费或明确废弃。
...
| 门槛 | 当前状态 | 说明 |
|---|---|---|
| 单元/契约回归 | 通过 | 147 个测试通过 |
| 架构依赖测试 | 通过 | 生产导入方向、单一 RRF、模型边界均有测试 |
...
- 删除内部 legacy 模块前必须完成仓库调用方扫描和评测迁移。
...
- [ ] Eval 不依赖内部 legacy ranking 或生产私有实现。
- [x] 旧公开 REST/MCP 路径和 schema 已直接删除。
